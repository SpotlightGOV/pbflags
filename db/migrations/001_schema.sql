...
-- Uses IF NOT EXISTS / IF NOT EXISTS guards so this migration is idempotent when
-- re-running after switching to the pbflags-specific goose version table.
...
CREATE INDEX IF NOT EXISTS idx_flags_killed ON feature_flags.flags(state) WHERE state = 'KILLED';
...
CREATE INDEX IF NOT EXISTS idx_overrides_entity ON feature_flags.flag_overrides(entity_id);
CREATE INDEX IF NOT EXISTS idx_overrides_killed ON feature_flags.flag_overrides(state) WHERE state = 'KILLED';
...
CREATE INDEX IF NOT EXISTS idx_audit_flag ON feature_flags.flag_audit_log(flag_id, created_at DESC);
